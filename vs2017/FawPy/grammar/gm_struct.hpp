...
struct obj_if_t : std::vector<std::pair<obj_value_t, std::vector<obj_stat_t>>> {
	...
	std::string to_str (ParseTypes &pt, size_t padding, std::string str_self);
};
...
struct obj_stat_t : public std::variant<obj_expr_t, obj_if_t, obj_for_t, obj_while_t, nullptr_t> {
	...
	std::string to_str (ParseTypes &pt, size_t padding, std::string str_self) {
		if (index () == 0) {
			return hStringA::format ("%s%s;\n", hStringA::get_padding (padding).c_str (), std::get<0> (*this).to_str (pt, str_self).c_str ());
		} else if (index () == 1) {
			return std::get<1> (*this).to_str (pt, padding, str_self);
		} else if (index () == 2) {
			return std::get<2> (*this).to_str (pt, padding, str_self);
		} else if (index () == 3) {
			return std::get<3> (*this).to_str (pt, padding, str_self);
		}
		return "";
	}

	std::string get_setval_varname (ParseTypes &pt) {
		if (index () != 0)
			return "";
		obj_expr_t &expr = std::get<0> (*this);
		if (expr.m_operator != "=")
			return "";
		return expr.m_vOper1[0].to_str (pt, "");
	}
};

std::string obj_if_t::to_str (ParseTypes &pt, size_t padding, std::string str_self) {
	std::string s = "";
	for (std::pair<obj_value_t, std::vector<obj_stat_t>> &p : *this) {
		s += hStringA::get_padding (padding);
		if (!s.empty ())
			s += "else ";
		std::string tmp = p.first.to_str (pt, str_self);
		if (!tmp.empty ()) {
			s += "if (";
			if (!tmp.empty () && tmp[0] == '(')
...
std::string obj_for_t::to_str (ParseTypes &pt, size_t padding, std::string str_self) {
	std::string s = hStringA::format ("%sfor (auto %s = %s, %s != %s; %s += %s) {\n", hStringA::get_padding (padding).c_str (),
		m_name.c_str (), m_begin.to_str (pt, str_self).c_str (),
		m_name.c_str (), m_end.to_str (pt, str_self).c_str (),
		m_name.c_str (), m_step.to_str (pt, str_self).c_str ());
	for (obj_stat_t &stat : m_vStat)
		s += stat.to_str (pt, padding + 1, str_self);
	s += hStringA::format ("%s}\n", hStringA::get_padding (padding).c_str ());
	return s;
}
std::string obj_while_t::to_str (ParseTypes &pt, size_t padding, std::string str_self) {
	std::string s = hStringA::format ("%swhile %s {\n", hStringA::get_padding (padding).c_str (), m_expr.to_str (pt, str_self).c_str ());
	for (obj_stat_t &stat : m_vStat)
		s += stat.to_str (pt, padding + 1, str_self);
	s += hStringA::format ("%s}\n", hStringA::get_padding (padding).c_str ());
	return s;
